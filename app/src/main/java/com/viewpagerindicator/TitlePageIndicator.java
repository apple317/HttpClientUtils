...
public class TitlePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final float halfWidth = getWidth() / count;
...
//                }
                ...
                if (currentPage && currentSelected) {
                   ...
                   // canvas.drawText(pageTitle, 0, pageTitle.length(), halfWidth * i + (halfWidth / 2 - (mPaintText.getTextSize() / 2)), bound.bottom + mTopPadding, mPaintText);
                    mPaintText.setTextAlign(Paint.Align.CENTER);
                    canvas.drawText(pageTitle+"",bound.left+(bound.right-bound.left)/2,  bound.bottom, mPaintText);

                    //  canvas.drawText(pageTitle, 0, pageTitle.length(), bound.left+(halfWidth/2-pageTitle.length()/2), bound.bottom + mTopPadding, mPaintText);
                }else{
                    mPaintText.setTextAlign(Paint.Align.CENTER);
                    canvas.drawText(pageTitle+"",bound.left+(bound.right-bound.left)/2,  bound.bottom, mPaintText);
...
//        mPath.close();
        ...
        switch (mFooterIndicatorStyle) {
            ...
            case Underline:
                ...
                final float rightPlusPadding = (page + 1) * halfWidth + mFooterIndicatorUnderlinePadding;
                final float leftMinusPadding = page * halfWidth - mFooterIndicatorUnderlinePadding;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (!mIsDragging) {
                    ...
                    Log.e("HU","ACTION_UP=====eventX="+eventX);
                    if (action != MotionEvent.ACTION_CANCEL) {
                        Log.e("HU","ACTION_UP====2222=eventX="+eventX);
                        mViewPager.setCurrentItem((int) ((eventX*5)/getWidth()));
                        return true;
                    }
//                    eventX/(getWidth()/5)
//
...
//                    }
    ...
    private Rect calcBounds(int index, Paint paint) {
        ...
        final int count = mViewPager.getAdapter().getCount();
        final int halfWidth = getWidth() / count;
        Rect bounds = new Rect(halfWidth * index, 0,halfWidth * index+ halfWidth, (int) (paint.descent() - paint.ascent()));
        return bounds;
    }
